...
public:
    ...
    void reset() {
        for (int i = 0; i < m * n; ++i) {
            out[i] = (T)(2.f * ((rand() / (float)INT_MAX) - 0.5f));
        }
        for (int i = 0; i < m * k; ++i) {
...
public:
    ...
    std::tuple<float, float, float> operator()() {
        ...
        hgemm::hgemm<T>(out, a, b, m, n, k, alpha, beta, 0);
        gpuDeviceSynchronize();
        gpuEventRecord(stop);
    ...
    bool validate(CPUInputs<T> &inputs, float atol) {
        ...
        for (int i = 0; i < m * n; ++i) {
            if (is_error(out_cpu[i], inputs.out[i], atol)) {
                val = false;
                std::cout << "\n>>> out:" << (float)out_cpu[i] << ", ref_out:" << (float)inputs.out[i] << "\n";
                break;
            }
        }
        delete[] out_cpu;
...
int main() {
    ...
    for (int i = 0; i < ms.size(); ++i) {
        ...
        auto [val, ms, gbps, tflops] = test::runbench<__half>(m, n, k, alpha, beta);
